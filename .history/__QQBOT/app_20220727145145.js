      ...
      case "!allconfigs" :
        ...
        API("GET", "configs").then(function(result) {
          ...
          for(i = 0; i < result.length - 1; i++) {
            ...
            string += "[NO." + JSON.parse(result).configs[i].name.toString() + "] - " + JSON.parse(result).configs[i].config_id.toString() + "\n";
            e.reply(string, true)
          }
        });
      break;
      case "!allscripts" :
        ...
        API("GET", "scripts").then(function(result) {
          for(i = 0; i < result.length - 1; i++) {
            ...
            if(JSON.parse(result).scripts[i].user_id == require("../manifest").QUID) continue;
