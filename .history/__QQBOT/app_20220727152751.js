  ...
  switch(parameters[0]) {
    case "!allconfigs" :
      ...
      API("GET", "configs").then(function(result) {
        if(JSON.parse(result).errors != undefined) {e.reply("ERROR: " + JSON.parse(result).errors.message, true);return;};
    ...
    case "!getconfigsubs" :
      ...
      API("GET", "configs/" + parameters[1]).then(function(result) {
        if(JSON.parse(result).errors != undefined) {e.reply("ERROR: " + JSON.parse(result).errors.message, true);return;};
        if(JSON.parse(result).config == undefined) {e.reply("ERROR: cannot resolve remote host package" + J, true);return;};
        if(JSON.parse(result).config.user_id != require("../manifest").userID) {e.reply("ERROR: cannot resolve remote host packag", true);return;};
        var string = "脚本 " + JSON.parse(result).config.name.toString() + "的订阅者信息：\n";
        string += "[" + JSON.parse(result).config.name.toString() + "] - " + JSON.parse(result).config.script_id.toString() + "\n";
        e.reply(string, true)
      });
    break;
    default:
