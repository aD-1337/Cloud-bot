  ...
  switch(parameters[0]) {
    ...
    case "!getconfigsubs" :
      ...
      API("GET", "configs/" + parameters[1]).then(function(result) {
        if(JSON.parse(result).errors != undefined) {e.reply("ERROR: " + JSON.parse(result).errors.message, true);return;};
        if(JSON.parse(result).config == undefined) {e.reply("ERROR: " + JSON.parse(result).errors.message, true);return;};
        if(JSON.parse(result).config.user_id != require("../manifest").userID) {e.reply("ERROR: " + JSON.parse(result).errors.message, true);return;};
