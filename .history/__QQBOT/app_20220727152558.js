  ...
  switch(parameters[0]) {
    ...
    case "!getconfigsubs" :
      ...
      API("GET", "configs/" + parameters[1]).then(function(result) {
        ...
        var string = "脚本 " + JSON.parse(result).config[i].name.toString() + "的订阅者信息：\n";
        for(i = 0; i < result.length - 1; i++) {
          if(JSON.parse(result).config[i] == undefined) continue;
          if(JSON.parse(result).config[i].user_id != require("../manifest").userID) continue;
          string += "[" + JSON.parse(result).config[i].name.toString() + "] - " + JSON.parse(result).scripts[i].script_id.toString() + "\n";
          e.reply(string, true)
        }
      });
    break;
