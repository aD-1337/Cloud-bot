    ...
    case "!allconfigs" :
      ...
      API("GET", "configs").then(function(result) {
        ...
        for(i = 0; i < result.length - 1; i++) {
          ...
          string += "[" + JSON.parse(result).configs[i].user_id == require("../manifest").userID && '""' + JSON.parse(result).configs[i].name.toString() + "] - " + JSON.parse(result).configs[i].config_id.toString() + "\n";
          e.reply(string, true)
        }
      });
    break;
