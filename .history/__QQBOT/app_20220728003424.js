...
var rateLimitRecorder = Date.parse(new Date());
client.on("system.online", () => console.log("Logged in!"))
client.on("message.group", e => {
  //合法性检查
  console.log(rateLimitRecorder - Date.parse(new Date()))
  if(rateLimitRecorder < Date.parse(new Date())) {
    rateLimitRecorder = Date.parse(new Date()) + 5 * 1000;
  } else {
    e.reply("操作过于频繁 请等待 " + ((rateLimitRecorder - Date.parse(new Date())) / 1000) + "秒 后重试", true)
    return;
  }
  ...
  if (e.raw_message != e.message[0].text) return;

  if (admin_list.match(e.user_id.toString())[0] != e.user_id.toString()) return;
  ...
  switch(parameters[0]) {
    ...
    case "!newconfigsub" :
      ...
      API("POST", "configs/"+encodeURI(parameters[1]) +"/subscriptions",{user_id:encodeURI(parameters[2])}).then(function(result) {
        ...
        e.reply("新增订阅-发票-----------\n" +
                "对象：" + JSON.parse(result).subscription.user_id + "\n" +
                "所在项目：" + JSON.parse(result).subscription.config_id + "\n" +
                "项目类型：CONFIG\n" + 
                "发起时间：" + new Date(JSON.parse(result).subscription.created * 1000).toLocaleString().replace(",",' ') + "\n" +
                "发起人：" + e.user_id.toString() + "\n" +
                "*请妥善保管好此信息用于后续查询*\n" +
                "请注意：PEDDING 状态的用户必须前往官网删除\n" +
                "-----------------------", true);
      });
    break;
    case "!delconfigsub" :
      ...
      API("DELETE", "configs/"+encodeURI(parameters[1]) +"/subscriptions",{user_id:encodeURI(parameters[2])}).then(function(result) {
        console.log(JSON.parse(result))
        if(JSON.parse(result).errors != undefined) {e.reply("ERROR: " + JSON.parse(result).errors[0].message + "\ntips:PEDDING 状态的用户必须前往官网删除", true);return;};
        if(JSON.parse(result).subscription == undefined) {e.reply("ERROR: cannot resolve remote host package", true);return;};
        e.reply("删除订阅-发票-----------\n" +
                "对象：" + JSON.parse(result).subscription.user_id + "\n" +
                "所在项目：" + JSON.parse(result).subscription.config_id + "\n" +
    ...
    case "!newscriptsub" :
      if(parameters.length != 3 || !parameters[1].match(new RegExp('^[0-9]+$')) || !parameters[2].match(new RegExp('^[0-9]+$'))) {e.reply("参数格式不正确", true); return;};
      API("POST", "scripts/"+encodeURI(parameters[1]) +"/subscriptions",{user_id:encodeURI(parameters[2])}).then(function(result) {
        console.log(JSON.parse(result))
        if(JSON.parse(result).errors != undefined) {e.reply("ERROR: " + JSON.parse(result).errors[0].message, true);return;};
        if(JSON.parse(result).subscription == undefined) {e.reply("ERROR: cannot resolve remote host package", true);return;};
        e.reply("新增订阅-发票-----------\n" +
                "对象：" + JSON.parse(result).subscription.user_id + "\n" +
                "所在项目：" + JSON.parse(result).subscription.script_id + "\n" +
                "项目类型：SCRIPT\n" + 
                "发起时间：" + new Date(JSON.parse(result).subscription.created * 1000).toLocaleString().replace(",",' ') + "\n" +
                "发起人：" + e.user_id.toString() + "\n" +
                "*请妥善保管好此信息用于后续查询*\n" +
    ...
    case "!delscriptsub" :
      if(parameters.length != 3 || !parameters[1].match(new RegExp('^[0-9]+$')) || !parameters[2].match(new RegExp('^[0-9]+$'))) {e.reply("参数格式不正确", true); return;};
      API("DELETE", "scripts/"+encodeURI(parameters[1]) +"/subscriptions",{user_id:encodeURI(parameters[2])}).then(function(result) {
        console.log(JSON.parse(result))
        if(JSON.parse(result).errors != undefined) {e.reply("ERROR: " + JSON.parse(result).errors[0].message + "\ntips:PEDDING 状态的用户必须前往官网删除", true);return;};
        if(JSON.parse(result).subscription == undefined) {e.reply("ERROR: cannot resolve remote host package", true);return;};
        e.reply("删除订阅-发票-----------\n" +
    ...
    case "!help":
      ...
      "1. !allconfigs\n"+
      "2. !allscripts\n"+
      "3. !queryconfig [config_id]\n"+
      "4. !queryscript [script_id]\n"+
      "5. !newconfigsub [config_id] [user_id]\n"+
      "6. !newconfigsub [config_id] [user_id]\n"+
      "7. !delconfigsub [config_id] [user_id]\n"+
      "8. !newscriptsub [config_id] [user_id]\n"+
      "9. !delscriptsub [config_id] [user_id]\n"+
      "-----------------------" , true);
    break;
    default:
      e.reply("此指令无效 输入 !help 获得帮助", true);
    break;
  }
})
...
client.on("system.offline.network", function (e) {
  //扫码后按回车登录
  process.stdin.once("data", () => {
    this.login()
  })
