...
client.on("message.group", e => {
  ...
  switch(parameters[0]) {
    ...
    case "!getconfigsubs" :
      ...
      API("GET", "configs/" + parameters).then(function(result) {
        if(JSON.parse(result).errors != undefined) {e.reply(JSON.parse(result).errora.message, true);return;};
        var string = "脚本 " + JSON.parse(result).scripts[i].name.toString() + "的订阅者信息：\n";
        for(i = 0; i < result.length - 1; i++) {
          if(JSON.parse(result).scripts[i] == undefined) continue;
          if(JSON.parse(result).scripts[i].user_id != require("../manifest").userID) continue;
          string += "[" + JSON.parse(result).scripts[i].name.toString() + "] - " + JSON.parse(result).scripts[i].script_id.toString() + "\n";
