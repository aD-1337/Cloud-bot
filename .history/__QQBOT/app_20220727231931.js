  ...
  switch(parameters[0]) {
    case "!allconfigs" :
      ...
      API("GET", "configs").then(function(result) {
        ...
        let ownned = JSON.parse(result).configs[i].user_id == require("../manifest").userID && '"OWNED" ' || '';
        for(i = 0; i < result.length - 1; i++) {
          if(JSON.parse(result).configs[i] == undefined) continue;
          string += "----------------------------\n"
          string += "[" + ownned + JSON.parse(result).configs[i].name.toString() + "] - " + JSON.parse(result).configs[i].config_id.toString() + "\n";
          e.reply(string, true)
        }
      });
    break;
