      ...
      case "!allconfigs" :
        ...
        API("GET", "configs").then(function(result) {
          for(i = 0; i < result.length - 1; i++) {
            ...
            string += "[NO." + (i + 1) + "] - " + JSON.parse(result).configs[i].config_id.toString() + "\n";
            e.reply(string, true)
          }
        });
      break;
      case "!allscripts" :
        ...
        API("GET", "scripts").then(function(result) {
          ...
          for(i = 0; i < result.length - 1; i++) {
            ...
            string += "[" + JSON.parse(result).scripts[i].script_id.toString() + "] - " + JSON.parse(result).scripts[i].script_id.toString() + "\n";
            e.reply(string, true)
          }
        });
      break;
