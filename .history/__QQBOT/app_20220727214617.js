...
client.on("message.group", e => {
  ...
  console.log(admin_list.match(e.user_id.toString())[0] != e.user_id.toString()) return;
  if (e.raw_message != e.message[0].text) {e.reply("您的消息存在编码错误", true); return;}
  if (!admin_list.match(e.user_id.toString())) return;
  ...
  switch(parameters[0]) {
    ...
    break;
    case "!getconfigsubs" :
      if(parameters.length != 2) {e.reply("参数格式不正确", true); return;};
      API("GET", "configs/" + parameters[1]).then(function(result) {
        if(JSON.parse(result).errors != undefined) {e.reply("ERROR: " + JSON.parse(result).errors[0].message, true);return;};
        ...
        var string = "配置 " + JSON.parse(result).config.name.toString() + "的订阅者信息：\n";
        for(i = 0; i < JSON.parse(result).config.subscriptions.length; i++) {
          string += "[" + (i + 1) + "] - " + JSON.parse(result).config.subscriptions[i].user_id + "\n";
        }
        string += "*空则为无*"
        e.reply(string, true)
      });
    ...
    case "!getscriptsubs" :
      if(parameters.length != 2) {e.reply("参数格式不正确", true); return;};
      API("GET", "scripts/" + parameters[1]).then(function(result) {
        if(JSON.parse(result).errors != undefined) {e.reply("ERROR: " + JSON.parse(result).errors[0].message, true);return;};
        if(JSON.parse(result).script == undefined) {e.reply("ERROR: cannot resolve remote host package", true);return;};
        if(JSON.parse(result).script.user_id != require("../manifest").userID) {e.reply("ERROR: you're not owning this config", true);return;};
        var string = "脚本 " + JSON.parse(result).script.name.toString() + "的订阅者信息：\n";
        for(i = 0; i < JSON.parse(result).script.subscriptions.length; i++) {
          string += "[" + (i + 1) + "] - " + JSON.parse(result).script.subscriptions[i].user_id + "\n";
        }
        string += "*空则为无*"
        e.reply(string, true)
      });
