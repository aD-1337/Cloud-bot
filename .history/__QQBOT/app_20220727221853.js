...
client.on("message.group", e => {
  ...
  switch(parameters[0]) {
    ...
    case "!getconfigsubs" :
      console.log("^[0-9]*$".test(parameters[1]) == false)
      if(parameters.length != 2 || "^[0-9]*$".test(parameters[1]) == false) {e.reply("参数格式不正确", true); return;};
    ...
    case "!createconigsub" :
      if(parameters.length != 2) {e.reply("参数格式不正确", true); return;};
      API("POST", "configs/"+ parameters[1] +"/subscriptions​").then(function(result) {
        consolee.log(result)
        e.reply("Please handle the next action.", true)
      });
    break;
    default:
