...
client.on("message.group", e => {
  ...
  switch(parameters[0]) {
    ...
    case "!createconfigsub" :
      if(parameters.length != 3 || !parameters[1].match(new RegExp('^[0-9]+$')) || !parameters[1].match(new RegExp('^[0-9]+$'))) {e.reply("参数格式不正确", true); return;};
      API("POST", "configs/"+ parameters[1] +"/subscriptions​", { name: "Script Name" }).then(function(result) {
        consolee.log(result)
